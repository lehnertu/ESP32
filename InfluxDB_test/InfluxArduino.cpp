...
{   
    ...
    http.addHeader("Content-Type", "application/json"); // not sure what influx is looking for but this works "text/plain"?
                                                  // maybe application/json ??
    if(_isAuthorised)
    {
    ...
    _latestResponse = http.POST((uint8_t *)writeBuf, strlen(writeBuf));
    http.end();
...
int InfluxArduino::ping()
{
    HTTPClient http;
    if(_isSecure)
    {
       http.begin(_host, _port, "/ping", _cert);
    }
    else
    {
        http.begin(_host, _port, "/ping");
    }
    http.addHeader("Content-Type", "text/plain"); // not sure what influx is looking for but this works?
                                                  // maybe application/json ??

    _latestResponse = http.GET();
    http.end();
    return _latestResponse;
}
